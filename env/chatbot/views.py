...
def message(request):
    ...
    nlp_request = ai.text_request()
    nlp_request.lang = 'de'
    nlp_request.session_id="123"
    nlp_request.query = msg_str

    nlp_response = nlp_request.getresponse()

    data = json.loads(nlp_response.read().decode('utf-8'))

    intent_name = str(data['result']['metadata']['intentName'])
    transportation = str(data['result']['parameters']['transportation'])

    if intent_name == "TimeSchedule":
